...
# Tag.py settings — tuned for Groq free tier (30 req/min, 14400 req/day on llama-3.1-8b-instant)
TAG_BATCH_SIZE = 250              # max rows to tag per daily run (~15 min)
TAG_DELAY_SECONDS = 2.5           # 24 req/min, safely under 30/min limit
TAG_MIN_TEXT_LENGTH = 15          # skip LLM for short noise like "Nope" / "Hi sir"
...
# --- Sentiment & topic tagging via Groq --------------------------------------

def tag_with_llm(review_text: str) -> dict:
    """Send a review to Groq (Llama 3.1 8B Instant) and get back sentiment + topic tags.
    ...
    system_msg = (
        "You classify product reviews and return strict JSON only. "
        "No prose, no markdown."
    )
    user_msg = f"""Classify this product review of JioTag (a Bluetooth item tracker similar to Apple AirTag, made by Reliance Jio in India).

Review: \"\"\"{review_text[:2000]}\"\"\"

Return ONLY a JSON object with these keys:
- sentiment: one of "positive", "neutral", "negative"
- sentiment_confidence: a number between 0.0 and 1.0
- topics: an array of tags chosen from this list only: {TOPIC_TAGS}
- competitor_mention: true if AirTag, Tile, SmartTag, or other competitor is named, else false"""
    ...
    for attempt in range(4):
        try:
            ...
            log(f"  Groq tagging error (attempt {attempt + 1}): {e}")
            if attempt < 3:
                time.sleep(backoff)
                backoff *= 2
            continue
